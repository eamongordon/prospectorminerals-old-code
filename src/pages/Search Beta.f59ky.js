...
function runSearch(term) {
    ...
    wixSearch.search()
        ...
        .then(async (results) => {
            ...
            /*
                const minerals = results.documents
                    .filter((obj) => obj.collectionNames[0] === 'Database')
                    .map((obj) => {
                        const path = obj.url.split('/').reverse()[0]
                        return {
                            "url": obj.url,
                ...
                const localities = results.documents
                    .filter((obj) => obj.collectionNames[0] === 'LocalityDatabase')
                    .map((obj) => {
                        const path = obj.title.split('-').reverse()[0]
                        return {
                            "url": obj.url,
                            "title": path.charAt(0).toUpperCase() + path.slice(1)
                        }
...
export function iTitle_input(event) {
    ...
    debounceTimer = setTimeout(() => {
        runSearch(event.target.value); //filter($w('#iTitle').value);
